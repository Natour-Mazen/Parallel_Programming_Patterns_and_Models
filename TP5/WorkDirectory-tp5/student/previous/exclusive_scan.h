...
namespace OPP {
    ...
    /**
     * @brief Performs an exclusive scan operation on an input range, using a specified binary function.
     *
     ...
     * @param Tinit The initial value for the scan operation, defaulting to 0.
     *
     * This function performs an exclusive scan operation on the input range [aBegin, aEnd),
     * using the binary function `functor` and the initial value `Tinit`. The result is written
     * to the output range beginning at `oBegin`. The scan operation is exclusive, meaning that
     * the i-th output element is the result of applying the binary function to the first i input elements.
     */
    ...
    inline void exclusive_scan(
      ...
      for (int i = 1; i < nbTasks - 1; ++i)
        aux[i] = functor(aux[i - 1], functor(oBegin[chunkSize * (i + 1) - 1],aBegin[chunkSize * (i + 1) - 1]));
